...
Repo root: `insight-cx/`
- `insight-cx/backend/` — existing Flask application (read-only, see below)
- `insight-cx/frontend/` — new React application (this is where you build)

Claude Code is run from `insight-cx/` so both directories are visible. Inspect `backend/` to understand routes and data, but only ever write files under `frontend/`.

## Development Environment
Before proposing the API/service layer, determine and report:
- What port Flask runs on locally, and how it's started
- Whether CORS is already enabled on the Flask side for local dev
- Whether the React dev server should proxy API calls to Flask, or call it via a full base URL
- Whether there is any authentication/session mechanism on the backend today, and if not, state that explicitly rather than assuming a login flow exists
...
## Authentication Note
If the backend has no authentication mechanism, Administration and other areas should be designed without login-gated UI for now. Do not fabricate a login screen or user-role system not present in the backend.
